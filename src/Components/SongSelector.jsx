    ...
    <div className="flex justify-center">
      <div className="relative p-3 w-full max-w-[400px]" ref={dropdownRef}>
        ...
        {isFocused && filteredSongs.length > 0 && (
          <ul
            className="absolute z-10 mt-2 w-full max-w-[375px] bg-white border border-gray-300 rounded-lg shadow-lg max-h-48 overflow-auto"
            role="listbox"
          >
            {filteredSongs.map((song) => {
              // Get the actual index from the full songBank
              const originalIndex = songBank.findIndex((s) => s.title === song.title);
